...
<html>
<head>
<title>Sales Order Report | Inventory and Distribution Management System</title>
...
<body>
  ...
  <center>
    ...
    <?php
        ...
        $sql_query = "SELECT t1.pms_id AS pms_hid, t1.pms_date, t1.pms_duedate, t1.pms_deliveryreceipt, t1.pms_purchaseorder,
                     t1.pms_customer, t1.pms_shippingmark, t1.pms_carriers, t1.pms_checkedby, t1.pms_salesman,
                     t1.pms_total AS h_pms_total, t1.pms_paid, t1.pms_settled, t1.pms_creditmemo, t1.pms_posted, t1.pms_posteddate,
        ...
        WHERE t1.pms_date >= '$pms_date_from' AND t1.pms_date <= '$pms_date_to' ORDER BY t1.pms_id ;";

        $sql = mysqli_query($GLOBALS["___mysqli_ston"], $sql_query);
        $report_count = 0;
        $print_pms_hid_temp = "";
        $print_pms_salesorder_temp = "";
        ...
        while ($rows=mysqli_fetch_array($sql)) {
            ...
            $print_pms_deliveryreceipt = $rows['pms_deliveryreceipt'] ;
            $print_pms_purchaseorder = $rows['pms_purchaseorder'] ;
            $print_pms_customer = $rows['pms_customer'] ;
            $print_pms_shippingmark = $rows['pms_shippingmark'] ;
            $print_pms_carriers = $rows['pms_carriers'] ;
            $print_pms_checkedby = $rows['pms_checkedby'] ;
            $print_pms_salesman = $rows['pms_salesman'] ;
            $print_pms_total = $rows['pms_total'] ;
            $print_pms_paid = $rows['pms_paid'] ;
            $print_pms_settled = $rows['pms_settled'] ;
            $print_pms_creditmemo = $rows['pms_creditmemo'] ;
            ...
            if ($print_pms_hid_temp!=$rows['pms_hid']) {
                ...
                '<hr><table>
                    ...
                    <tbody>
                        <tr>
                        <td> <strong>Sales Order #: </strong>'. $print_pms_hid . '</td>
                        <td> Due Date:  '. $print_pms_duedate . '</td>
                        <td> SO Date:  '. $print_pms_date . '</td>
                        <td> DR #:  '. $print_pms_deliveryreceipt . '</td>
                        </tr>

                        <tr>
                        <td>PO #:  '. $print_pms_purchaseorder . '</td>
                        <td>Carriers:  '. $print_pms_carriers . '</td>
                        <td>Customer:  '. $print_pms_customer . '</td>
                        <td>Checked by:  '. $print_pms_checkedby . '</td>
                        </tr>

                        <tr>
                        <td>Shippingmark:  '. $print_pms_shippingmark . '</td>
                        <td>Salesman:  '. $print_pms_salesman . '</td>
                        <td>Settled:  '. $print_pms_settled . '</td>
                        <td>Paid:  '. $print_pms_paid . '</td>
                        </tr>

                        <tr>
                        <td>Credit Memo:  '. $print_pms_creditmemo . '</td>
                        <td>Reference:  '. $print_pms_reference . '</td>
                ...
                if ($print_pms_salesorder_temp!=$rows['pms_salesorder']) {
                    echo $report_detail ;
                }
                echo
                '<table>
            ...
            $print_pms_salesorder_temp = $rows['pms_salesorder'] ;
